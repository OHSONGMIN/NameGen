...
public class NicknameService {
    ...
    public ResponseDTO generateGameNickname(String keyword) {
        ...
        String system = "너는 재치 넘치고 기발한 게임 닉네임을 만들어내는 최고의 생성기야. 사용자 요청에 반드시 JSON 배열 형태로만 응답해야 해. 설명 없이 배열만 출력해야 해.";
        String user = "\\\"%s\\\"에서 연상되는 단어를 활용해 2~6글자 닉네임 5개를 추천해줘. 의외의 조합이나 언어유희를 써도 좋아. 말장난, 줄임말, 짧은 소리 위주로 만들어줘. 살짝 B급 감성도 괜찮아. 단, 귀엽거나 임팩트 있게. 출력 형식은 반드시 [\\\"닉1\\\", \\\"닉2\\\", \\\"닉3\\\", \\\"닉4\\\", \\\"닉5\\\"] 만 보여줘."
                .formatted(keyword);

        return new ResponseDTO(requestNicknamesFromOpenAi(system, user));
    }

    public ResponseDTO generateInstagramNickname(String keyword) {

        String system = "너는 감각적이고 센스 있는 인스타그램 아이디를 만들어주는 최고의 생성기야. 사용자 요청에 반드시 JSON 배열 형태로만 응답해야 해. 설명 없이 배열만 출력해야 해.";
    ...
    public ResponseDTO generateCarNickname(String car, String keyword) {
        ...
        String system = "너는 창의적이고 감각적인 자동차 애칭을 만들어주는 최고의 생성기야. 사용자 요청에 반드시 JSON 배열 형태로만 응답해야 해. 설명 없이 배열만 출력해야 해.";
        ...
        String user = "\\\"%s\\\"을(를) 바탕으로 자동차 애칭 5개를 추천해줘. 줄임말, 말장난, 센스 있는 조합도 괜찮고, 짧고 인상 깊은 이름이면 더 좋아. 출력은 반드시 [\\\"애칭1\\\", \\\"애칭2\\\", \\\"애칭3\\\", \\\"애칭4\\\", \\\"애칭5\\\"] 형식의 JSON 배열만 보여줘."
                .formatted(condition);
